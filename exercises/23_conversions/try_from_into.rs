...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        if !check_range(tuple.0) { return Err(IntoColorError::IntConversion); }
        if !check_range(tuple.1) { return Err(IntoColorError::IntConversion); }
        if !check_range(tuple.2) { return Err(IntoColorError::IntConversion); }
        fn check_range(pix: i16) -> bool {
            if pix < 0 || pix > 255 {
                return false;
            }
            true
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        ...
        for val in arr {
            if val < 0 || val > 255 { return Err(IntoColorError::IntConversion); }
        }
        Ok(Self {
            red: arr[0] as u8,
            green: arr[1] as u8,
